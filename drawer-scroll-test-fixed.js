...
(async () => {
  ...
  try {
    ...
    const todoCount = await page.$$eval('.simple-do-item', items => items.length);
    ...
    // 查找完成记录按钮 - 在简单待办页面中
    const historyToggle = await page.$('button[data-panel-target="history-panel"], #simple-do-history-toggle, button[aria-label*="历史"], button[aria-label*="history"]');
    if (!historyToggle) {
      throw new Error('未找到完成记录按钮');
    }
